...
— ถ้าจำเป็นต้องแก้ ให้แจ้ง Layout Lead ก่อน
"""
...
def render(ctx):
        ...
        if single_member_sector:
            ...
            position_label = (
                ...
                if sector_rank == 1
                    ...
                    "STRONG COMPETITOR"
                    if sector_rank <= max(2, n_sector // 2)
                    else "LAGGING PEER"
                )
            )
        ...
        st.markdown(
            ...
            ">
                ...
                <p style="
                    ...
                    margin:0;
                ">
                    อันดับที่ {sector_rank} จาก {n_sector}
                    บริษัทในกลุ่ม {ctx.stock_info.get('sector','-')}
                    จาก Overall Score =
                    {safe(ctx.stock_info.get('overall_score')):.1f}/100
                </p>
            </div>
            """,
            unsafe_allow_html=True
        ...
        def build_dims(df):
            ...
            for label, col, color in dims_cols:
                ...
                if (
                    ...
                    and ctx.stock_info.get('revenue_growth_yoy') is None
                ):
                    pct = 50
                else:
                    pct = calc_pct(df, col)
        ...
        if single_member_sector:
            ...
            ">
                <div style="
                    ...
                    line-height:1.4;
                ">
                    กลุ่ม <b>{ctx.stock_info.get('sector','-')}</b>
                    มีเพียง 1 หุ้น
                    จึงไม่สามารถเปรียบเทียบ percentile
                    ภายในกลุ่มได้อย่างมีความหมาย
                </div>
            </div>
            """
            ...
            ">
                ...
                <div>
                    <div style="
                        ...
                        margin-bottom:6px;
                    ">
                        เปรียบเทียบกับหุ้นทั้ง {n_all} ตัว
                    </div>
    ...
    with r2_c1:
        ...
        peers_sorted = ctx.sector_peers.sort_values(
            'overall_score',
            ascending=False
        )
            ...
            for th, lab, col in zip(
                ...
                if val >= th:
                    return (
                        ...
                        f'font-weight:bold;">{lab}</span>'
                    )

            return (
                f'<span style="color:{colors[-1]};">'
                f'{labels[-1]}</span>'
            )
        ...
        st.markdown(
            ...
            ">
                ...
                <div style="
                    ...
                    margin-top:6px;
                ">
                    *จัดอันดับจาก Overall Score
                    ที่คำนวณจริงจากข้อมูลใน cis_summary_scores
                </div>
            </div>
            """,
            unsafe_allow_html=True
    ...
    with r2_c3:
        ...
        # เพิ่มกรอบการ์ดให้ Strategic Matrix
        with st.container(border=True):
            show_chart(
                fig_matrix,
                key="industry_matrix",
    ...
    with r3_c1:
        ...
        compare_df = (
            ctx.scores_df
            if single_member_sector
            else ctx.sector_peers
        )

        strengths_ib, weaknesses_ib = [], []

        if (
            ctx.stock_info['health_score']
            > compare_df['health_score'].mean()
        ):
            strengths_ib.append(
                "Health Score สูงกว่าค่าเฉลี่ย"
            )

        if (
            ctx.stock_info['ai_score']
            > compare_df['ai_score'].mean()
        ):
            strengths_ib.append(
                "AI Prediction Score สูงกว่าค่าเฉลี่ย"
            )
        ...
        if (
            ...
            strengths_ib.append(
                f"รายได้เติบโต "
                f"{safe(ctx.stock_info.get('revenue_growth_yoy')):.1f}% YoY"
            )

        if not strengths_ib:
            strengths_ib.append(
                "ผลประกอบการยังอยู่ระหว่างพัฒนาเทียบกลุ่ม"
            )

        if (
            ctx.stock_info['valuation_score']
            < compare_df['valuation_score'].mean()
        ):
            weaknesses_ib.append(
                "Valuation แพงกว่าค่าเฉลี่ย"
            )

        if (
            ctx.stock_info['risk_score']
            < compare_df['risk_score'].mean()
        ):
            weaknesses_ib.append(
                "ความเสี่ยงสูงกว่าค่าเฉลี่ย"
            )
        ...
        if (
            ...
            weaknesses_ib.append(
                "AI Prediction Score ต่ำกว่าค่าเฉลี่ย"
            )

        if not weaknesses_ib:
            weaknesses_ib.append(
                "ไม่พบจุดอ่อนเชิงเปรียบเทียบที่ชัดเจน"
            )
    ...
    with r3_c2:
        ...
        compare_desc = (
            f"เทียบกับทั้ง {n_all} หุ้นที่ติดตาม "
            f"(กลุ่มมีตัวเดียว)"
            if single_member_sector
        ...
        st.markdown(
            ...
            ">
                ...
                ">
                    ...
                    อยู่อันดับที่ <b>{sector_rank}</b>
                    จาก {n_sector} บริษัทในกลุ่ม
                    {ctx.stock_info.get('sector','-')}
                    (Overall Score
                    {safe(ctx.stock_info.get('overall_score')):.1f}/100)
                    {compare_desc}:
                ...
                ">
                    <div>
                        ...
                        Health Score:
                        {safe(ctx.stock_info.get('health_score')):.1f}
                        ({avg_label}
                        {compare_df['health_score'].mean():.1f})
                    </div>

                    <div>
                        <span style="color:#10B981;">✔</span>
                        Valuation Score:
                        {safe(ctx.stock_info.get('valuation_score')):.1f}
                        ({avg_label}
                        {compare_df['valuation_score'].mean():.1f})
                    </div>

                    <div>
                        <span style="color:#10B981;">✔</span>
                        AI Prediction Score:
                        {safe(ctx.stock_info.get('ai_score')):.1f}
                        ({avg_label}
                        {compare_df['ai_score'].mean():.1f})
                    </div>

                    <div>
                        <span style="color:#10B981;">✔</span>
                        Risk Score:
                        {safe(ctx.stock_info.get('risk_score')):.1f}
                        ({avg_label}
                        {compare_df['risk_score'].mean():.1f})
                    </div>
                </div>
            </div>
            """,
    ...
    download_link = (
        f'data:file/csv;base64,{b64_csv}'
    )
    ...
    st.markdown(
        ...
        ">
            <div style="
                ...
                color:#475569;
            ">
                <b>EXPORT & INDUSTRY DATA</b>
                <br>
                <span style="
                    color:#64748B;
                    font-size:13px;
                ">
                    ดาวน์โหลดคะแนนวิเคราะห์ทั้งหมดของ
                    {ctx.selected_ticker}
                    (ข้อมูลจริงจาก cis_summary_scores)
                </span>
            </div>

            <a
                href="{download_link}"
                download="{ctx.selected_ticker}_CIS_Analysis.csv"
                style="
                    background:#3B82F6;
                    color:white;
                    border:none;
