...
def render(ctx):
    ...
    def hl_card(icon, bg, label, value, sub, val_color="#0F172A"):
        return f"""
        <div style="background-color:{'#DCFCE7' if rec in ['STRONG BUY','BUY'] else '#ECFCCB' if rec == 'ACCUMULATE' else '#FEE2E2' if rec == 'REDUCE / SELL' else '#FEF3C7'};border:1px solid #E2E8F0;border-radius:8px;
