...
def render(ctx):
    ...
    with col_left:
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px 12px 0 0; padding:16px 16px 8px 16px;">
    ...
    <div style="font-size:14.5px; color:#64748B; margin-top:2px;">{COMPANY_NAMES.get(ctx.selected_ticker,'-')}</div>
    <div style="display:flex; align-items:baseline; gap:8px; margin-top:10px;">
    <span style="font-size:28px; font-weight:bold; color:#0F172A; line-height:1;">{ctx.current_price:.2f}</span>
    ...
    <div style="font-size:13px; color:#64748B; margin-top:4px;">Dataset close &bull; {ctx.stock_info.get('latest_date','-')}</div>
        ...
        fcf_yield = (
            safe(ctx.stock_info.get('free_cash_flow_latest'))
            / (safe(ctx.stock_info.get('market_cap_mb')) * 1e6) * 100
        ) if safe(ctx.stock_info.get('market_cap_mb')) > 0 else 0

        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-top:none; border-radius:0 0 12px 12px; padding:8px 16px 16px 16px;">
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; border-top:1px solid #E2E8F0; padding-top:10px;">
    <div>
        <div style="font-size:13px; color:#64748B;">Market Cap</div>
        <div style="font-size:15px; font-weight:bold; color:#0F172A; margin-top:2px;">{fmt_mb(safe(ctx.stock_info.get('market_cap_mb'))*1e6)}</div>
    </div>
    <div>
        <div style="font-size:13px; color:#64748B;">P/E (TTM)</div>
        <div style="font-size:15px; font-weight:bold; color:#0F172A; margin-top:2px;">{fmt_ratio(ctx.stock_info.get('pe_ratio'))}</div>
    </div>
    <div>
        <div style="font-size:13px; color:#64748B;">Sector</div>
        <div style="font-size:15px; font-weight:bold; color:#0F172A; margin-top:2px;">{ctx.stock_info.get('sector','-').split(' ')[0]}</div>
    </div>
    <div>
        <div style="font-size:13px; color:#64748B;">P/B (TTM)</div>
        <div style="font-size:15px; font-weight:bold; color:#0F172A; margin-top:2px;">{fmt_ratio(ctx.stock_info.get('pb_ratio'))}</div>
    </div>
    <div>
        <div style="font-size:13px; color:#64748B;">Industry</div>
        <div style="font-size:14.5px; font-weight:bold; color:#0F172A; margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{ctx.stock_info.get('sector','-')}</div>
    </div>
    <div>
        <div style="font-size:13px; color:#64748B;">FCF Yield</div>
        <div style="font-size:15px; font-weight:bold; color:#10B981; margin-top:2px;">{fcf_yield:.2f}%</div>
    </div>
    </div>
    </div>""", unsafe_allow_html=True)
    ...
    with col_center:
        ...
        def module_card(num, label, score, color, badge, desc, badge_bg):
            return f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:10px; padding:16px 14px; text-align:center; position:relative;">
    ...
    <div style="margin:0 auto 10px auto; width:88px; height:88px; border-radius:50%; background:conic-gradient({color} 0% {score}%, #E2E8F0 {score}% 100%); display:flex; align-items:center; justify-content:center;">
        <div style="width:72px; height:72px; border-radius:50%; background-color:#FFFFFF; display:flex; flex-direction:column; align-items:center; justify-content:center;">
            <span style="font-size:20px; font-weight:bold; color:#0F172A; line-height:1;">{score}</span>
    ...
    <div style="font-size:14.5px; color:#475569; line-height:1.4;">{desc}</div>
        ...
        cards_html = "".join([
            module_card(
                "01", "COMPANY HEALTH", m1_s, "#34D399",
                m1_badge, m1_desc, "rgba(16,185,129,0.2)"
            ),
            module_card(
                "02", "FAIR VALUE", m2_s, "#FBBF24",
                m2_badge, m2_desc, "rgba(245,158,11,0.2)"
            ),
            module_card(
                "03", "ENTRY TIMING", m3_s, "#38BDF8",
                m3_badge, m3_desc, "rgba(56,189,248,0.2)"
            ),
            module_card(
                "04", "AI PREDICTION", m4_s, "#C084FC",
                m4_badge, m4_desc, "rgba(168,85,247,0.2)"
            ),
            module_card(
                "05", "RISK ANALYSIS", m5_s, "#FB923C",
                m5_badge, m5_desc, "rgba(249,115,22,0.2)"
            ),
            module_card(
                "06", "INDUSTRY BENCHMARK", m6_s, "#2DD4BF",
                m6_badge, m6_desc, "rgba(20,184,166,0.2)"
            ),
        ])

        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; padding:16px; min-height:900px;">
    <div style="font-size:15px; font-weight:bold; color:#0F172A; letter-spacing:0.5px; margin-bottom:12px;">
        INVESTMENT DECISION OVERVIEW ({ctx.selected_ticker})
    </div>

    <div class="overview-module-grid" style="display:grid; grid-template-columns:repeat(2, 1fr); gap:12px;">
        {cards_html}
    </div>
    </div>""", unsafe_allow_html=True)
    ...
    with col_right:
        ...
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; padding:16px; min-height:900px; display:flex; flex-direction:column; justify-content:space-between; text-align:center;">
    <div>

    <div style="font-size:14.5px; font-weight:bold; color:#64748B; letter-spacing:0.5px; text-align:left; margin-bottom:4px;">
    ...
    <div style="font-size:13px; font-weight:bold; color:#64748B; margin-top:2px;">
        OVERALL SCORE
    </div>
    ...
    <div style="color:{rec_color}; font-size:16px; font-weight:bold; margin-top:2px;">
        {label}
    </div>

    <div style="font-size:12.5px; color:#475569; line-height:1.35; margin-top:4px; padding:0 2px;">
    <b>{ctx.selected_ticker}</b> ได้คะแนนภาพรวม {overall:.1f}/100
    จุดเด่นหลักอยู่ที่ {top_strength}
    อันดับ {int(ctx.stock_info.get('sector_rank',1))} จาก {n_sector}
    บริษัทในกลุ่ม {ctx.stock_info.get('sector','-')}
    </div>

    <div style="background-color:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:8px 10px; margin-top:8px; text-align:left;">

    <div style="font-size:12.5px; color:#64748B; font-weight:bold; margin-bottom:2px;">
        RECOMMENDATION
    </div>
    ...
    <div>
        <b style="color:{rec_color}; font-size:16px; line-height:1;">{rec}</b>
    ...
    def hl_card(icon, bg, label, value, sub, val_color="#0F172A"):
        return f"""<div style="background-color:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:10px 12px; display:flex; align-items:center; gap:10px;">
    <div style="background:{bg}; width:40px; height:40px; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:17.5px;">
        {icon}
    </div>

    <div>
        <div style="font-size:13px; color:#64748B;">{label}</div>
        <div style="font-size:16px; font-weight:bold; color:{val_color}; margin-top:1px;">{value}</div>
    ...
    st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; padding:14px 16px;">

    <div style="font-size:14.5px; font-weight:bold; color:#64748B; margin-bottom:10px; letter-spacing:0.5px;">
        KEY HIGHLIGHTS
    </div>
