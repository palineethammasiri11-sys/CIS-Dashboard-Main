...
def render(ctx):
    ...
    # ============================================================
    # 2) PREDICTION — 2 กล่อง: ซ้าย = gauge % / ขวา = กล่องคำอธิบาย
    # ============================================================
...
<text x="50" y="47" text-anchor="middle" font-size="20" font-weight="bold" fill="#FFFFFF">{prob_up:.0f}%</text>
</svg>
</div>
    ...
    with exp_c1:
        if not fi.empty:
            ...
            fig_shap.update_layout(
                ...
                yaxis=dict(
                    tickfont=dict(size=13.5, color="#CBD5E1"),
                    gridcolor="#1E293B",
                    zeroline=False
                ),
                showlegend=False
...
<div style="font-size:13.5px; color:{MUTED}; border-top:1px solid #1E293B; padding-top:10px; margin-top:10px;">Validation: Out-of-time (Train 2023-24 / Test 2025)</div>
        ...
        if not bt.empty:
            ...
            fig_bt.update_layout(
                ...
                legend=dict(
                    ...
                    xanchor="left",
                    x=0,
                    font=dict(size=12.5, color="#CBD5E1")
                )
            )
            ...
            st.markdown(
                f"""<div style="background:#0F172A; border:1px solid #1E293B; border-top:none; border-radius:0 0 12px 12px; padding:8px 14px 12px 14px; font-size:13.5px; color:{MUTED};">* Test-set Accuracy: {acc_val:.1f}%</div>""",
                unsafe_allow_html=True
            )
    ...
    with st.expander("📋 รายละเอียดโมเดลและข้อมูล (Model & Data Detail)"):
        ...
        st.markdown(
            f"""<ul style="font-size:16px; line-height:1.9; color:#CBD5E1; margin:0; padding-left:22px;">
