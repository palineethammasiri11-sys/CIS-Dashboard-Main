...
RED = "#EF4444"
BLUE = "#0284C7"
MUTED = "#64748B"  # สีข้อความรองสำหรับธีมสว่าง
...
def _kpi_card(label, value_html, sub_html="", value_color="#0F172A", value_size=36, border="#D9E2EC"):
    """การ์ด KPI ใบเดียว (HTML บรรทัดเดียว ห้ามมีบรรทัดว่าง/เยื้องหน้า ไม่งั้น markdown จะตีเป็น code block)"""
    return (
        f'<div style="background-color:#FFFFFF; border:1px solid {border}; border-radius:12px; padding:18px 10px; '
...
def render(ctx):
    ...
    st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #D9E2EC; border-top:none; border-radius:0 0 12px 12px; padding:16px;">
...
<div style="font-size:18px; color:#334155; line-height:1.7;">โมเดล Random Forest ประเมินว่า <b>{ctx.selected_ticker}</b> มีโอกาส<b style="color:{status_color};">{direction_th}</b> <b>{dir_prob:.0f}%</b> ในอีก 10 วันทำการ</div>
<div style="font-size:18px; color:#334155; line-height:1.7;">ด้วยความแม่นยำการทดสอบ <b>{acc_val:.1f}%</b> ({baseline_note}เกณฑ์เปรียบเทียบ {baseline_val:.1f}%) &nbsp;→&nbsp; คำแนะนำ: <b style="color:{status_color};">{signal}</b></div>
<div style="font-size:14.5px; color:{MUTED}; line-height:1.6; margin-top:12px;">โปรดใช้ประกอบการตัดสินใจลงทุน ควรพิจารณาร่วมกับ Fair Value และ Company Health ก่อนตัดสินใจ ไม่ใช่คำแนะนำโดยตรง</div>{warn_line}
    ...
    with exp_c1:
        if not fi.empty:
            ...
            fig_shap.update_layout(
                ...
                yaxis=dict(
                    tickfont=dict(size=13.5, color="#334155"),
                    gridcolor="#D9E2EC",
                    zeroline=False
                ),
                showlegend=False
    ...
    with exp_c2:
        top_feat = (
            fi.sort_values('importance', ascending=False).iloc[0]['feature']
            if not fi.empty
            else "N/A"
        )
...
<p style="font-size:17px; color:#334155; line-height:1.7; margin:0;">โมเดลใช้ 6 ตัวชี้วัดเชิงเทคนิคในการทำนาย โดย feature ที่มีอิทธิพลต่อผลทำนายของ <b>{ctx.selected_ticker}</b> สูงสุดคือ
<b style="color:{BLUE};">{top_feat}</b> — ค่านี้มาจากน้ำหนักจริงที่ Random Forest เรียนรู้ได้ ไม่ใช่ค่าคงที่</p>
    ...
    # ============================================================
    st.markdown(
        _section_title("📊 MODEL PERFORMANCE (TEST SET 2025, actual)"),
        unsafe_allow_html=True
    )
...
<div style="font-size:13.5px; color:{MUTED}; border-top:1px solid #D9E2EC; padding-top:10px; margin-top:10px;">Validation: Out-of-time (Train 2023-24 / Test 2025)</div>
</div>""",
            unsafe_allow_html=True
        )
    ...
    with perf_c2:
        st.markdown(
            _section_title("HISTORICAL PREDICTION PERFORMANCE (Test Set, actual)"),
            unsafe_allow_html=True
        )
        ...
        if not bt.empty:
            ...
            fig_bt.update_layout(
                height=235,
                margin=dict(l=30, r=30, t=5, b=18),
                paper_bgcolor="#FFFFFF",
                plot_bgcolor="#FFFFFF",
                xaxis=dict(
                ...
                legend=dict(
                    ...
                    xanchor="left",
                    x=0,
                    font=dict(size=12.5, color="#334155")
                )
            )
            ...
            st.markdown(
                f"""<div style="background:#FFFFFF; border:1px solid #D9E2EC; border-top:none; border-radius:0 0 12px 12px; padding:8px 14px 12px 14px; font-size:13.5px; color:{MUTED};">* Test-set Accuracy: {acc_val:.1f}%</div>""",
                unsafe_allow_html=True
            )
        else:
            st.markdown(
    ...
    with st.expander("📋 รายละเอียดโมเดลและข้อมูล (Model & Data Detail)"):
        n_train = len(
            ctx.stock_daily[ctx.stock_daily['date'] < '2025-01-01']
        )
        n_test = len(
            ctx.stock_daily[ctx.stock_daily['date'] >= '2025-01-01']
        )
