...
<div><span style="font-size:11px; font-weight:bold; color:{MUTED}; letter-spacing:0.5px;">{text}</span></div></div>"""
...
def _kpi_card(
    ...
    return (
        ...
        f'<div style="font-size:10px; font-weight:bold; color:{label_color}; letter-spacing:1px;">{label}</div>'
...
def _kpi_sub(text, color=MUTED, bold=False):
    ...
    return f'<div style="font-size:10px; font-weight:{weight}; color:{color}; margin-top:2px;">{text}</div>'
...
def _metric_cell(label, value):
    return (
        ...
        f'<div style="font-size:10.5px; color:{MUTED};">{label}</div>'
        f'<div style="font-size:18px; font-weight:bold; color:#0F172A; line-height:1.35;">{value}</div></div>'
    )
...
def render(ctx):
    ...
    <div style="margin-bottom:20px;">
        <div style="font-size:17px; font-weight:700; color:#0F172A; letter-spacing:0.3px;">
            AI PREDICTION
        </div>
...
""",
    ...
    if reliability_low:
        warn_line = (
            f'<div style="font-size:10.5px; color:{RED}; background:rgba(239,68,68,0.1); border:1px solid {RED}; '
...
<text x="50" y="47" text-anchor="middle" font-size="20" font-weight="bold" fill="#0F172A">{prob_up:.0f}%</text>
</svg>
</div>
...
<div style="font-size:10.5px; font-weight:bold; color:{MUTED}; letter-spacing:1px; margin-bottom:10px;">
PROBABILITY OF UP
</div>
...
<div style="font-size:10.5px; color:{MUTED}; line-height:1.6; margin-top:12px;">
...
<div style="font-size:12px; font-weight:bold; color:{MUTED}; letter-spacing:0.5px; margin-bottom:12px;">
...
<div style="font-size:11px; color:{RED if reliability_low else GREEN}; border-top:1px dashed #D9E2EC; padding-top:12px; margin-top:14px; line-height:1.55;">
vs. Baseline (naive majority-class): <b>{baseline_val:.1f}%</b> — {"ต่ำกว่า baseline ⚠" if reliability_low else "สูงกว่า baseline ✓"}
</div>
<div style="font-size:10px; color:{MUTED}; border-top:1px solid #D9E2EC; padding-top:10px; margin-top:10px;">
Validation: Out-of-time (Train 2023-24 / Test 2025)
    ...
    with perf_c2:
        ...
        if not bt.empty:
            ...
            st.markdown(
                f"""<div style="background:#FFFFFF; border:1px solid #D9E2EC; border-top:none; border-radius:0 0 12px 12px; padding:8px 14px 12px 14px; font-size:10px; color:{MUTED};">
* Test-set Accuracy: {acc_val:.1f}%
    ...
    with st.expander(
        ...
        st.markdown(
            f"""<ul style="font-size:11px; line-height:1.9; color:#334155; margin:0; padding-left:22px;">
