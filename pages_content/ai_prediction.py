...
<div><span style="font-size:13.5px; font-weight:bold; color:{MUTED}; letter-spacing:0.5px;">{text}</span></div></div>"""


def _kpi_card(label, value_html, sub_html="", value_color="#F8FAFC", value_size=30, border="#1E293B"):
    ...
    return (
        ...
        f'<div style="font-size:11px; font-weight:bold; color:{MUTED}; letter-spacing:1px;">{label}</div>'
...
def _kpi_sub(text, color=MUTED, bold=False):
    ...
    return f'<div style="font-size:11.5px; font-weight:{weight}; color:{color}; margin-top:2px;">{text}</div>'
...
def _metric_cell(label, value):
    return (
        ...
        f'<div style="font-size:11.5px; color:{MUTED};">{label}</div>'
        f'<div style="font-size:21px; font-weight:bold; color:#F8FAFC; line-height:1.35;">{value}</div></div>'
    )
...
def render(ctx):
    ...
    with k4:
        st.markdown(_kpi_card(
            "SCORE",
            f'{ai_score}<span style="font-size:15px; color:{MUTED};">/100</span>',
...
<text x="50" y="47" text-anchor="middle" font-size="21" font-weight="bold" fill="#FFFFFF">{prob_up:.0f}%</text>
</svg>
</div>
...
<div style="font-size:11px; font-weight:bold; color:{MUTED}; letter-spacing:1px; margin-bottom:10px;">PROBABILITY OF UP</div>
<div style="font-size:14px; color:#E2E8F0; line-height:1.7;">โมเดล Random Forest ประเมินว่า <b>{ctx.selected_ticker}</b> มีโอกาส<b style="color:{status_color};">{direction_th}</b> <b>{dir_prob:.0f}%</b> ในอีก 10 วันทำการ</div>
<div style="font-size:14px; color:#E2E8F0; line-height:1.7;">ด้วยความแม่นยำการทดสอบ <b>{acc_val:.1f}%</b> ({baseline_note}เกณฑ์เปรียบเทียบ {baseline_val:.1f}%) &nbsp;→&nbsp; คำแนะนำ: <b style="color:{status_color};">{signal}</b></div>
<div style="font-size:11.5px; color:{MUTED}; line-height:1.6; margin-top:12px;">โปรดใช้ประกอบการตัดสินใจลงทุน ควรพิจารณาร่วมกับ Fair Value และ Company Health ก่อนตัดสินใจ ไม่ใช่คำแนะนำโดยตรง</div>{warn_line}
</div>
</div>
    ...
    with exp_c2:
        ...
        st.markdown(
            f"""<div style="background-color:#0F172A; border:1px solid #1E293B; border-radius:12px; padding:22px; min-height:310px; display:flex; flex-direction:column; justify-content:center;">
<div style="font-size:13px; font-weight:bold; color:{MUTED}; letter-spacing:0.5px; margin-bottom:12px;">
...
— ค่านี้มาจากน้ำหนักจริงที่ Random Forest เรียนรู้ได้ ไม่ใช่ค่าคงที่
    ...
    with perf_c1:
        ...
        st.markdown(
            f"""<div style="background-color:#0F172A; border:1px solid #1E293B; border-top:none; border-radius:0 0 12px 12px; padding:16px; min-height:350px;">
<div style="display:grid; grid-template-columns: repeat(2, 1fr); gap:10px;">{cells}</div>
<div style="font-size:12px; color:{RED if reliability_low else GREEN}; border-top:1px dashed #1E293B; padding-top:12px; margin-top:14px; line-height:1.55;">
vs. Baseline (naive majority-class): <b>{baseline_val:.1f}%</b> — {"ต่ำกว่า baseline ⚠" if reliability_low else "สูงกว่า baseline ✓"}
</div>
<div style="font-size:11px; color:{MUTED}; border-top:1px solid #1E293B; padding-top:10px; margin-top:10px;">
Validation: Out-of-time (Train 2023-24 / Test 2025)
    ...
    with perf_c2:
        ...
        if not bt.empty:
            ...
            st.markdown(
                f"""<div style="background:#0F172A; border:1px solid #1E293B; border-top:none; border-radius:0 0 12px 12px; padding:8px 14px 12px 14px; font-size:11px; color:{MUTED};">
* Test-set Accuracy: {acc_val:.1f}%
            ...
            st.markdown(
                """<div style="background:#0F172A; border:1px solid #1E293B; border-top:none; border-radius:0 0 12px 12px; padding:20px;">""",
                unsafe_allow_html=True
            )
    ...
    with st.expander(
        ...
        st.markdown(
            f"""<ul style="font-size:12.5px; line-height:1.9; color:#CBD5E1; margin:0; padding-left:22px;">
