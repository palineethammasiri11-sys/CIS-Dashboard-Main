...
def render(ctx):
    ...
    <div><div style="display:flex; align-items:center; gap:8px;"><h2 style="margin:0; font-size:23px; font-weight:bold; color:#0F172A; letter-spacing:0.5px;">FAIR VALUE ASSESSMENT</h2></div>
    ...
    <div style="color:#0284C7; font-size:13.5px; font-weight:bold;">Lower Estimate</div><div style="color:#64748B; font-size:12px;">Min(DCF, P/E)</div>
    ...
    <div style="color:#7C3AED; font-size:13.5px; font-weight:bold;">Blended Fair Value</div><div style="color:#64748B; font-size:12px;">55% DCF + 45% P/E</div>
    ...
    <b style="color:#0F172A;">DCF Fair Value: {safe(ctx.stock_info.get('dcf_fair_value')):.2f} THB &nbsp;|&nbsp; P/E Fair Value: {safe(ctx.stock_info.get('pe_fair_value')):.2f} THB</b><br>
    ...
    <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b style="color:#0F172A;">Fair Value (Blended): {val_base:.2f} THB</b><br><span style="color:#64748B; font-size:12.5px;">ประเมินแบบผสมผสาน DCF + Relative P/E</span></div></div>
    <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b style="color:#0F172A;">Margin of Safety: {val_mos:.1f}%</b><br><span style="color:#64748B; font-size:12.5px;">ส่วนต่างความปลอดภัยจากราคาตลาดปัจจุบัน</span></div></div>
    <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b style="color:#0F172A;">P/E Ratio ปัจจุบัน: {fmt_ratio(ctx.stock_info.get('pe_ratio'), suffix='')} เท่า</b><br><span style="color:#64748B; font-size:12.5px;">เทียบ EPS ล่าสุด {ctx.stock_info.get('eps','-')} บาท/หุ้น</span></div></div>
    <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b style="color:#0F172A;">สถานะมูลค่า: {val_status}</b><br><span style="color:#64748B; font-size:12.5px;">ระดับความน่าดึงดูดเชิงมูลค่าพื้นฐาน</span></div></div>
    </div></div>""", unsafe_allow_html=True)
    ...
    <tr style="border-bottom:1px solid #E2E8F0;"><td style="padding:3px 0;">P/E Ratio ปัจจุบัน</td><td style="color:#0F172A; font-weight:bold;">{fmt_ratio(ctx.stock_info.get('pe_ratio'))}</td></tr>
    <tr><td style="padding:3px 0;">P/E Fair Value</td><td style="color:#0F172A; font-weight:bold;">{safe(ctx.stock_info.get('pe_fair_value')):.2f} THB</td></tr>
    ...
    with d_c2:
        ...
        <tr style="border-bottom:1px solid #E2E8F0;"><td style="padding:3px 0;">WACC</td><td style="color:#0F172A; font-weight:bold;">{fmt_ratio(val_detail.get('wacc_used'), suffix="%", decimals=1)}</td></tr>
        <tr><td style="padding:3px 0;">DCF Fair Value</td><td style="color:#0F172A; font-weight:bold;">{safe(ctx.stock_info.get('dcf_fair_value')):.2f} THB</td></tr>
        </table></div></div>""",
    ...
    <table style="width:100%; font-size:13px; color:#475569; border-collapse:collapse; margin-top:6px;">
    <tr style="border-bottom:1px solid #E2E8F0;"><td style="padding:3px 0;">Current</td><td style="color:#0F172A; font-weight:bold;">{val_cur_price:.2f}</td></tr>
    <tr style="border-bottom:1px solid #E2E8F0;"><td style="padding:3px 0;">Fair Value</td><td style="color:#0F172A; font-weight:bold;">{val_base:.2f}</td></tr>
    <tr><td style="padding:3px 0;">Difference</td><td style="color:#0F172A; font-weight:bold;">{(val_base - val_cur_price):+.2f}</td></tr>
    </table></div></div>""", unsafe_allow_html=True)
    ...
    <div style="font-size:13.5px; font-weight:bold; color:#64748B; letter-spacing:0.5px;">HISTORICAL FAIR VALUE VS PRICE (Actual, year-end 2023-2025)</div></div>""", unsafe_allow_html=True)
        ...
        if not fv_hist.empty:
            ...
            fig_hist_val.add_trace(go.Scatter(x=fv_hist['year'].astype(str), y=fv_hist['fair_value'], mode='lines+markers', name='Fair Value', line=dict(color='#8B5CF6', width=1.8, dash='dash')))
            fig_hist_val.add_trace(go.Scatter(x=fv_hist['year'].astype(str), y=fv_hist['price'], mode='lines+markers', name='Actual Price', line=dict(color='#0284C7', width=2), marker=dict(size=9, color='#0284C7')))
            fig_hist_val.update_layout(
                height=190, margin=dict(l=25, r=15, t=10, b=55), paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF",
                yaxis=dict(tickfont=dict(size=11.5, color="#64748B"), gridcolor="#E2E8F0", zeroline=False),
