...
def render(ctx):
    ...
    with r1_c1:
        st.markdown(
            f"""<div style="background-color:#FFFFFF; border:2px solid {val_color}; border-radius:12px; padding:14px; min-height:170px; display:flex; flex-direction:column; justify-content:space-between;">
        <div style="font-size:14px; font-weight:bold; color:#64748B; letter-spacing:0.5px;">FAIR VALUE SUMMARY</div>
        ...
        <span style="font-size:20px; font-weight:bold; color:#0F172A; line-height:1;">{val_score}</span><span style="font-size:13px; color:#64748B;">/100</span></div></div>
        <div><div style="color:{val_color}; font-size:17px; font-weight:bold; line-height:1.2;">{val_status}</div>
        <div style="font-size:14px; color:#475569; line-height:1.35; margin-top:3px;">Margin of Safety อยู่ที่ {val_mos:.1f}% เมื่อเทียบกับมูลค่าพื้นฐานที่แท้จริง</div>
        <div style="color:{val_color}; font-size:15px; letter-spacing:1px; margin-top:4px;">{'★'*val_stars}{'☆'*(5-val_stars)}</div></div>
        </div></div>""",
            unsafe_allow_html=True
        )
    ...
    with r1_c2:
        ...
        <div style="font-size:14px; font-weight:bold; color:#64748B;">CURRENT PRICE</div>
        <div><div style="font-size:22px; font-weight:bold; color:#0F172A; line-height:1;">{val_cur_price:.2f} <span style="font-size:14px; color:#64748B;">THB</span></div>
        <div style="font-size:13px; color:#64748B; margin-top:2px;">({ctx.stock_info.get('latest_date','-')})</div></div>
        </div>""",
            unsafe_allow_html=True
        )
    ...
    with r1_c3:
        st.markdown(
            f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; padding:12px; min-height:170px; display:flex; flex-direction:column; justify-content:space-between;">
        <div style="font-size:14px; font-weight:bold; color:#64748B;">ESTIMATED FAIR VALUE<br><span style="font-size:13px; color:#64748B;">(BLENDED: 55% DCF + 45% P/E)</span></div>
        <div><div style="font-size:22px; font-weight:bold; color:#0F172A; line-height:1;">{val_base:.2f} <span style="font-size:14px; color:#64748B;">THB</span></div></div>
        </div>""",
            unsafe_allow_html=True
        )
    ...
    with r1_c4:
        ...
        <div style="font-size:14px; font-weight:bold; color:#64748B; text-align:left;">MARGIN OF SAFETY</div>
        <div><div style="font-size:22px; font-weight:bold; color:{val_color}; line-height:1;">{val_mos:.1f}%</div></div>
        <div style="margin-top:auto; display:flex; justify-content:center;"><div style="background:rgba(16,185,129,0.15); border:1px solid #10B981; border-radius:50%; width:36px; height:36px; display:flex; align-items:center; justify-content:center; font-size:16px; color:#10B981;">🛡️</div></div>
        </div>""",
            unsafe_allow_html=True
        )
    ...
    with r1_c5:
        ...
        st.markdown(
            f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; padding:12px; min-height:170px; display:flex; flex-direction:column; justify-content:space-between; text-align:center;">
        <div style="font-size:14px; font-weight:bold; color:#64748B; text-align:left;">CONFIDENCE LEVEL</div>
        <div><div style="font-size:19px; font-weight:bold; color:{val_color}; line-height:1;">{conf.upper()}</div></div>
        </div>""",
            unsafe_allow_html=True
        )
    ...
    with r1_c6:
        st.markdown(
            f"""<div style="background-color:{val_color}; border:1px solid {val_color}; border-radius:12px; padding:12px; min-height:170px; display:flex; flex-direction:column; justify-content:space-between; text-align:center;">
        <div style="font-size:14px; font-weight:bold; color:#FFFFFF; text-align:left;">RECOMMENDATION</div>
        <div><div style="font-size:18px; font-weight:bold; color:#FFFFFF; line-height:1.1;">{val_rec_label}</div></div>
        </div>""",
            unsafe_allow_html=True
        )
    ...
    with r2_c1:
        ...
        <div style="color:#F59E0B; font-size:14px; font-weight:bold;">Blended Fair Value</div><div style="color:#64748B; font-size:13px;">55% DCF + 45% P/E</div>
        ...
        <span style="color:#10B981; font-size:15px;">✔</span><div style="font-size:13px; color:#475569; line-height:1.3;">
    ...
    with r2_c2:
        ...
        <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b>Fair Value (Blended): {val_base:.2f} THB</b><br><span style="color:#64748B; font-size:13px;">ประเมินแบบผสมผสาน DCF + Relative P/E</span></div></div>
        <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b>Margin of Safety: {val_mos:.1f}%</b><br><span style="color:#64748B; font-size:13px;">ส่วนต่างความปลอดภัยจากราคาตลาดปัจจุบัน</span></div></div>
        <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b>P/E Ratio ปัจจุบัน: {fmt_ratio(ctx.stock_info.get('pe_ratio'), suffix='')} เท่า</b><br><span style="color:#64748B; font-size:13px;">เทียบ EPS ล่าสุด {ctx.stock_info.get('eps','-')} บาท/หุ้น</span></div></div>
        <div style="display:flex; gap:6px;"><span style="color:#10B981;">✔</span><div><b>สถานะมูลค่า: {val_status}</b><br><span style="color:#64748B; font-size:13px;">ระดับความน่าดึงดูดเชิงมูลค่าพื้นฐาน</span></div></div>
        </div></div>""",
            unsafe_allow_html=True
        )
    ...
    with r2_c3:
        ...
        <div style="background:#E2E8F0; height:9px; border-radius:4px; overflow:hidden;"><div style="background:{val_color}; width:{val_score}%; height:100%;"></div></div></div>
        <div><div style="display:flex; justify-content:space-between; font-size:14px; color:#475569; margin-bottom:3px;"><span>🛡️ Margin of Safety</span><span style="font-weight:bold; color:#0F172A;">{safety_score} <span style="font-size:13px; color:#64748B;">/100</span></span></div>
