วันจันทร์ 21:54
อันนี้เอากรอบเทาเหมือนเดิม
วันนี้ 10:35
ข้อความที่วาง (1)(4).txt
เอกสาร
แก้ฟอนต์ให้ใหย่ขึ้น
ข้อความที่วาง (1)(5).txt
เอกสาร
อยากได้ฟ้อนต์ตามมนี้อ่ะ
ข้างล่างมันยังไม่ค่อยเปลี่ยนเลยอะ่

ใช่ อันก่อนผมปรับไม่ถึงทั้งหน้า 😭 รอบนี้ผมแก้ ทั้งหน้าใหม่หมด ไม่ใช่แค่ส่วนล่าง

ปรับแล้ว:

KPI ด้านบนใหญ่ขึ้น
ENTRY TIMING ANALYSIS ใหญ่ขึ้น
Score / Status / Confluence ใหญ่ขึ้น
SIGNAL CHECKLIST และรายการข้างในใหญ่ขึ้น
Chart title / ตัวเลข / legend ใหญ่ขึ้น
PRICE SETUP ใหญ่ขึ้น
SYSTEM SCORING METHODOLOGY ใหญ่ขึ้น
RISK / REWARD ใหญ่ขึ้น
WHY NOW? / WHY WAIT? ใหญ่ขึ้น
ข้อความในการ์ดด้านล่างใหญ่ขึ้นทั้งหมด
ไม่เปลี่ยน logic, calculation, สี หรือ layout
เช็ก syntax แล้ว ไม่มี error
โค้ดเต็มตัวใหม่

ดาวน์โหลด entry_timing.py — ปรับฟอนต์ทั้งหน้า

เอาไฟล์นี้ไปแทน pages_content/entry_timing.py ได้เลย

ตัวนี้ไม่ใช่การเพิ่มเฉพาะข้างล่างแล้ว แต่ปรับ typography ทั้งหน้าให้เป็นสเกลเดียวกันจริง ๆ
...
def render(ctx):
    ...
    <div style="font-size:15px; color:#64748B; margin-top:2px;">ประเมินมูลค่าที่เหมาะสมของหุ้นโดยใช้แบบจำลอง DCF ผสาน P/E Relative</div></div>
    ...
    with r1_c1:
        st.markdown(f"""<div style="background-color:#FFFFFF; border:2px solid {val_color}; border-radius:12px; padding:14px; min-height:170px; display:flex; flex-direction:column; justify-content:space-between;">
    ...
    <div><div style="font-size:21px; font-weight:bold; color:#0F172A; line-height:1;">{val_cur_price:.2f} <span style="font-size:13.5px; color:#64748B;">THB</span></div>
    ...
    with r1_c4:
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; padding:12px; min-height:170px; display:flex; flex-direction:column; justify-content:space-between; text-align:center;">
    ...
    <div style="font-size:13px; font-weight:bold; color:#FFFFFF; text-align:left;">RECOMMENDATION</div>
    <div><div style="font-size:17.5px; font-weight:bold; color:#FFFFFF; line-height:1.1;">{val_rec_label}</div></div>
    </div>""", unsafe_allow_html=True)
    ...
    st.markdown("<div style='margin-top:22px;'></div>", unsafe_allow_html=True)
    st.markdown("""<div style="font-size:14.5px; font-weight:bold; color:#64748B; letter-spacing:0.5px; margin-bottom:8px;">DETAIL BREAKDOWN</div>""", unsafe_allow_html=True)

    d_c1, d_c2, d_c3, d_c4 = st.columns(4)

    with d_c1:
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:10px; padding:12px; min-height:190px; display:flex; flex-direction:column; justify-content:space-between;">
    <div><div style="font-size:13px; font-weight:bold; color:#475569;">RELATIVE VALUATION (P/E)</div>
    ...
    with d_c2:
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:10px; padding:12px; min-height:190px; display:flex; flex-direction:column; justify-content:space-between;">
    <div><div style="font-size:13px; font-weight:bold; color:#475569;">INTRINSIC VALUATION (DCF)</div>
    <table style="width:100%; font-size:13px; color:#475569; border-collapse:collapse; margin-top:6px;">
    <tr style="border-bottom:1px solid #E2E8F0; color:#64748B; font-size:12.5px;"><th style="text-align:left; padding:2px 0;">Metric</th><th>Value</th></tr>
    ...
    with d_c3:
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:10px; padding:12px; min-height:190px; display:flex; flex-direction:column; justify-content:space-between;">
    <div><div style="font-size:13px; font-weight:bold; color:#475569;">PRICE COMPARISON</div>
    <table style="width:100%; font-size:13px; color:#475569; border-collapse:collapse; margin-top:6px;">
    ...
    with d_c4:
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:10px; padding:12px; min-height:190px; display:flex; flex-direction:column; justify-content:space-between;">
    <div><div style="font-size:13px; font-weight:bold; color:#475569;">MARGIN OF SAFETY</div>
    <table style="width:100%; font-size:13px; color:#475569; border-collapse:collapse; margin-top:6px;">
    ...
    with r4_c1:
        ...
        st.markdown(f"""<div style="background-color:#FFFFFF; border:1px solid #E2E8F0; border-radius:12px; padding:14px; min-height:220px; display:flex; flex-direction:column; justify-content:space-between;">
    <div style="font-size:13.5px; font-weight:bold; color:#64748B; letter-spacing:0.5px;">DCF ASSUMPTIONS <span style="font-size:11.5px; color:#64748B; font-weight:normal;">({ctx.stock_info.get('sector','-')})</span></div>
    ...
    <div style="font-size:11px; color:#64748B; border-top:1px solid #E2E8F0; padding-top:6px; margin-top:4px;">WACC/Growth ปรับตามกลุ่มอุตสาหกรรม (ไม่ใช่ค่าคงที่เดียวทุกหุ้นแล้ว) &bull; FCF ฐานใช้ค่าเฉลี่ย 2 ปีล่าสุด</div>
    </div>""", unsafe_allow_html=True)
    ...
    <div style="font-size:13.5px; font-weight:bold; color:#64748B; letter-spacing:0.5px;">HISTORICAL FAIR VALUE VS PRICE (Actual, year-end 2023-2025)</div></div>""", unsafe_allow_html=True)
    ...
    render_nav_footer("m2", prev_page=" Company Health", next_page=" Entry Timing")                                                                                      โมดุลต่อไป

ได้ โมดูลต่อไปคือ Fair Value (fair_value.py) 👍

ตัวนี้จากโค้ดที่ส่งมา โครงฟอนต์ใกล้กับ Company Health อยู่แล้ว แต่ยังมีบางจุดเล็กกว่า เช่น 11px / 11.5px / 12px ในรายละเอียด, assumptions และกราฟ ทำให้เวลาเปิดทั้งหน้าแล้วบางส่วนดูเล็กไม่เท่ากัน

รอบนี้ผมจะยึดสเกลเดียวกับที่เราเพิ่งปรับ Entry Timing:
...
body/detail → 13px
ข้อความรอง → 12px
ตัวเลขหลัก → 17–21px ตามระดับของข้อมูล
ตัวเลขในกราฟ → 12px
legend → 12px
ไม่แตะ calculation
ไม่แตะ data
...
และจุดที่ผมจะ ขยายเป็นพิเศษ คือ DETAIL BREAKDOWN, DCF ASSUMPTIONS และส่วน HISTORICAL FAIR VALUE VS PRICE เพราะในโค้ดปัจจุบันสามส่วนนี้มีฟอนต์เล็กกว่าส่วนบนชัดเจน

ส่งโมดูลถัดไปมาได้เลย เดี๋ยวผมไล่ปรับ ทีละโมดูลให้ฟอนต์ทั้ง Dashboard ไปในสเกลเดียวกัน.
...
def render(ctx):
    ...
    render_nav_footer("m2", prev_page=" Company Health", next_page=" Entry Timing")                                                                     นี่ไง

ใช่ นี่แหละ fair_value.py ตัวเต็มที่ต้องแก้ 😭
เมื่อกี้ผมเข้าใจว่าให้ดูเป็นตัวอย่างเฉย ๆ

ผมจะปรับจากโค้ด ตัวที่คุณส่งมานี้โดยตรง ไม่ตัดส่วนไหนออก และไม่เปลี่ยน logic/data/layout โดยเน้นให้ฟอนต์ทั้งหน้าอยู่ในสเกลเดียวกับ Company Health:
...
ข้อความรอง 12px
ตัวเลขสำคัญ 17–21px
ตารางรายละเอียดไม่ให้เล็ก
DCF Assumptions ไม่ให้เล็ก
กราฟไม่ให้ตัวหนังสือจิ๋ว
"""
pages_content/fair_value.py
-----------------------
หน้า "Fair Value" ของ CIS Dashboard
