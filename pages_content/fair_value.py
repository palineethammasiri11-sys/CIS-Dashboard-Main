...
ข้อมูลที่ใช้ได้ใน ctx:
    ctx.selected_ticker, ctx.stock_info, ctx.stock_daily, ctx.fin_stock,
    ctx.sector_peers, ctx.scores_df, ctx.fin_df, ctx.feat_imp_df,
    ctx.backtest_df, ctx.risk_hist_df, ctx.health_yearly_df,
    ctx.fair_value_yearly_df, ctx.current_price, ctx.change_pct,
    ctx.change_val, ctx.change_color, ctx.change_sign, ctx.arrow_sign
...
def render(ctx):
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:15px;">
        <div>
            <h2 style="margin:0;font-size:23px;font-weight:bold;color:#0F172A;letter-spacing:.5px;">FAIR VALUE</h2>
    ...
    # TOP SUMMARY
    # =========================================================
    r1_c1, r1_c2, r1_c3, r1_c4, r1_c5, r1_c6 = st.columns([1.5, .9, .9, .9, .9, 1.1])
    val_stars = min(5, max(1, round(val_score / 20)))

    with r1_c1:
        html(f"""
        <div style="background:#FFF;border:2px solid {val_color};border-radius:12px;padding:14px;min-height:170px;display:flex;flex-direction:column;justify-content:space-between;">
            <div style="font-size:13.5px;font-weight:bold;color:#64748B;letter-spacing:.5px;">FAIR VALUE SUMMARY</div>
            <div style="display:flex;align-items:center;gap:12px;">
                <div style="width:76px;height:76px;border-radius:50%;background:conic-gradient({val_color} 0% {val_score}%,#E2E8F0 {val_score}% 100%);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    <div style="width:62px;height:62px;border-radius:50%;background:#FFF;display:flex;flex-direction:column;align-items:center;justify-content:center;">
                        <span style="font-size:19px;font-weight:bold;color:#0F172A;line-height:1;">{val_score}</span>
                        <span style="font-size:12px;color:#64748B;">/100</span>
                    </div>
                </div>
                <div>
                    <div style="color:{val_color};font-size:16.5px;font-weight:bold;line-height:1.2;">{val_status}</div>
                    ...
                    <div style="color:{val_color};font-size:14.5px;letter-spacing:1px;margin-top:4px;">{'★' * val_stars}{'☆' * (5 - val_stars)}</div>
                </div>
            </div>
        </div>
        """)
    ...
    with r1_c2:
        html(f"""
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:12px;padding:12px;min-height:170px;display:flex;flex-direction:column;justify-content:space-between;">
            <div style="font-size:13px;font-weight:bold;color:#64748B;">CURRENT PRICE</div>
            <div>
                <div style="font-size:21px;font-weight:bold;color:#0F172A;line-height:1;">{val_cur_price:.2f} <span style="font-size:13.5px;color:#64748B;">THB</span></div>
    ...
    with r1_c3:
        html(f"""
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:12px;padding:12px;min-height:170px;display:flex;flex-direction:column;justify-content:space-between;">
            <div style="font-size:13px;font-weight:bold;color:#64748B;">ESTIMATED FAIR VALUE<br><span style="font-size:12px;color:#64748B;">(BLENDED: 55% DCF + 45% P/E)</span></div>
            <div><div style="font-size:21px;font-weight:bold;color:#0F172A;line-height:1;">{val_base:.2f} <span style="font-size:13.5px;color:#64748B;">THB</span></div></div>
        </div>
        """)

    with r1_c4:
        html(f"""
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:12px;padding:12px;min-height:170px;display:flex;flex-direction:column;justify-content:space-between;text-align:center;">
            <div style="font-size:13px;font-weight:bold;color:#64748B;text-align:left;">MARGIN OF SAFETY</div>
            ...
            <div style="margin-top:auto;display:flex;justify-content:center;">
                <div style="background:rgba(16,185,129,.15);border:1px solid #10B981;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:15px;color:#10B981;">🛡️</div>
            </div>
        </div>
        """)
    ...
    with r1_c5:
        ...
        html(f"""
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:12px;padding:12px;min-height:170px;display:flex;flex-direction:column;justify-content:space-between;text-align:center;">
            <div style="font-size:13px;font-weight:bold;color:#64748B;text-align:left;">CONFIDENCE LEVEL</div>
            <div><div style="font-size:18.5px;font-weight:bold;color:{val_color};line-height:1;">{conf.upper()}</div></div>
        </div>
        """)

    with r1_c6:
        html(f"""
        <div style="background:{val_color};border:1px solid {val_color};border-radius:12px;padding:12px;min-height:170px;display:flex;flex-direction:column;justify-content:space-between;text-align:center;">
            <div style="font-size:13px;font-weight:bold;color:#FFF;text-align:left;">RECOMMENDATION</div>
            <div><div style="font-size:17.5px;font-weight:bold;color:#FFF;line-height:1.1;">{val_rec_label}</div></div>
        </div>
        """)
        ...
        <div style="background:#FFFFFF;border:1px solid #E2E8F0;border-radius:12px;padding:14px;min-height:315px;display:flex;flex-direction:column;justify-content:space-between;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1.1fr 1fr;gap:6px;margin-top:6px;">
                <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:8px 4px;text-align:center;">
                    <div style="color:#EF4444;font-size:13.5px;font-weight:bold;">Lower Estimate</div>
                    <div style="color:#64748B;font-size:12px;">Min(DCF, P/E)</div>
                    <div style="color:#0F172A;font-size:16px;font-weight:bold;margin-top:4px;">{val_bear:.2f} <span style="font-size:12px;color:#64748B;">THB</span></div>
                </div>

                <div style="background:#FFFBEB;border:1.5px solid #F59E0B;border-radius:8px;padding:8px 4px;text-align:center;">
                    <div style="color:#F59E0B;font-size:13.5px;font-weight:bold;">Blended Fair Value</div>
                    <div style="color:#64748B;font-size:12px;">55% DCF + 45% P/E</div>
                    <div style="color:#0F172A;font-size:16.5px;font-weight:bold;margin-top:4px;">{val_base:.2f} <span style="font-size:12px;color:#64748B;">THB</span></div>
                </div>

                <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:8px 4px;text-align:center;">
                    <div style="color:#10B981;font-size:13.5px;font-weight:bold;">Upper Estimate</div>
                    <div style="color:#64748B;font-size:12px;">Max(DCF, P/E)</div>
                    <div style="color:#0F172A;font-size:16px;font-weight:bold;margin-top:4px;">{val_bull:.2f} <span style="font-size:12px;color:#64748B;">THB</span></div>
                </div>
            </div>
            ...
            <div style="display:flex;flex-direction:column;gap:10px;margin:auto 0;">
                <div>
                    <div style="display:flex;justify-content:space-between;font-size:13px;color:#475569;margin-bottom:3px;">
                        ...
                        <span style="font-weight:bold;color:#0F172A;">{val_score} <span style="font-size:12px;color:#64748B;">/100</span></span>
                    </div>
                    <div style="background:#E2E8F0;height:9px;border-radius:4px;overflow:hidden;"><div style="background:{val_color};width:{val_score}%;height:100%;"></div></div>
                </div>
                ...
                <div>
                    <div style="display:flex;justify-content:space-between;font-size:13px;color:#475569;margin-bottom:3px;">
                        ...
                        <span style="font-weight:bold;color:#0F172A;">{safety_score} <span style="font-size:12px;color:#64748B;">/100</span></span>
                    </div>
                    <div style="background:#E2E8F0;height:9px;border-radius:4px;overflow:hidden;"><div style="background:{val_color};width:{safety_score}%;height:100%;"></div></div>
                </div>
            </div>

            <div style="display:flex;justify-content:space-between;align-items:center;border-top:1px solid #E2E8F0;padding-top:8px;">
                <span style="font-size:13.5px;font-weight:bold;color:#475569;">OVERALL FAIR VALUE SCORE</span>
                <span style="font-size:18.5px;font-weight:bold;color:{val_color};">{val_score} <span style="font-size:13px;color:#64748B;">/100</span></span>
            </div>
        </div>
        """)
    ...
    st.markdown("<div style='margin-top:22px;'></div>", unsafe_allow_html=True)
    html("""
    <div style="font-size:14.5px;font-weight:bold;color:#64748B;letter-spacing:.5px;margin-bottom:8px;">
        DETAIL BREAKDOWN
    </div>
    """)
    ...
    with d_c1:
        html(f"""
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:10px;padding:12px;min-height:190px;display:flex;flex-direction:column;justify-content:space-between;">
            <div>
                <div style="font-size:13px;font-weight:bold;color:#475569;">RELATIVE VALUATION (P/E)</div>
                <table style="width:100%;font-size:13px;color:#475569;border-collapse:collapse;margin-top:6px;">
                    <tr style="border-bottom:1px solid #E2E8F0;color:#64748B;font-size:12.5px;"><th style="text-align:left;padding:2px 0;">Metric</th><th>Value</th></tr>
    ...
    with d_c2:
        ...
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:10px;padding:12px;min-height:190px;display:flex;flex-direction:column;justify-content:space-between;">
            <div>
                <div style="font-size:13px;font-weight:bold;color:#475569;">INTRINSIC VALUATION (DCF)</div>
                <table style="width:100%;font-size:13px;color:#475569;border-collapse:collapse;margin-top:6px;">
                    <tr style="border-bottom:1px solid #E2E8F0;color:#64748B;font-size:12.5px;"><th style="text-align:left;padding:2px 0;">Metric</th><th>Value</th></tr>
                    <tr style="border-bottom:1px solid #E2E8F0;"><td style="padding:3px 0;">WACC</td><td>{fmt_ratio(ctx.stock_info.get('wacc_used'), suffix="%", decimals=1)}</td></tr>
    ...
    with d_c3:
        ...
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:10px;padding:12px;min-height:190px;display:flex;flex-direction:column;justify-content:space-between;">
            <div>
                <div style="font-size:13px;font-weight:bold;color:#475569;">PRICE COMPARISON</div>
                <table style="width:100%;font-size:13px;color:#475569;border-collapse:collapse;margin-top:6px;">
    ...
    with d_c4:
        ...
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:10px;padding:12px;min-height:190px;display:flex;flex-direction:column;justify-content:space-between;">
            <div>
                <div style="font-size:13px;font-weight:bold;color:#475569;">MARGIN OF SAFETY</div>
                <table style="width:100%;font-size:13px;color:#475569;border-collapse:collapse;margin-top:6px;">
    ...
    with r4_c1:
        ...
        html(f"""
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:12px;padding:14px;min-height:220px;display:flex;flex-direction:column;justify-content:space-between;">
            <div style="font-size:13.5px;font-weight:bold;color:#64748B;letter-spacing:.5px;">
                DCF ASSUMPTIONS <span style="font-size:11.5px;color:#64748B;font-weight:normal;">({ctx.stock_info.get('sector','-')})</span>
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:13px;color:#475569;margin:auto 0;">
                ...
                <div><span style="color:#64748B;">Target P/E</span><br><b style="color:#0F172A;">18-22x (by sector)</b></div>
            ...
            <div style="font-size:11px;color:#64748B;border-top:1px solid #E2E8F0;padding-top:6px;margin-top:4px;">
                WACC/Growth ปรับตามกลุ่มอุตสาหกรรม (ไม่ใช่ค่าคงที่เดียวทุกหุ้นแล้ว) &bull; FCF ฐานใช้ค่าเฉลี่ย 2 ปีล่าสุด
            </div>
        </div>
        """)
    ...
    with r4_c2:
        html("""
        <div style="background:#FFF;border:1px solid #E2E8F0;border-radius:12px 12px 0 0;padding:12px 16px 0;">
            <div style="font-size:13.5px;font-weight:bold;color:#64748B;letter-spacing:.5px;">
                HISTORICAL FAIR VALUE VS PRICE (Actual, year-end 2023-2025)
            </div>
        </div>
        """)
