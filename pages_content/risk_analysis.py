...
def render(ctx):
    risk_score = int(
        round(safe(ctx.stock_info.get("risk_score"), 45))
    )

    risk_status = (
        "LOW RISK"
        if risk_score >= 65
        else ("MODERATE RISK" if risk_score >= 40 else "HIGH RISK")
    )

    risk_color = (
        "#10B981"
        if risk_score >= 65
        else ("#F59E0B" if risk_score >= 40 else "#EF4444")
    )
...
">
    ...
    <div style="margin:auto 0;">
        <svg
            viewBox="0 0 100 55"
            style="width:140px; height:90px; display:block; margin:0 auto;"
        >
            <path
    ...
    with r1_c2:
        ...
        def risk_dim_card(label, val):
            c = (
                "#10B981"
                if val <= 35
                else ("#F59E0B" if val <= 60 else "#EF4444")
            )

            lvl = (
                "Low"
                if val <= 35
                else ("Moderate" if val <= 60 else "High")
            )
...
""",
        ...
        beta_cmp = ctx.scores_df[
            ["ticker", "beta"]
        ].sort_values("beta")

        # หุ้นที่เลือก = น้ำเงิน
        # หุ้นอื่น = เทา
        colors_beta = [
...
<div style="
    ...
    padding-top:6px;
">
    VaR = ขาดทุนสูงสุดที่คาดใน 95% ของวัน (Parametric) |
...
""",
    ...
    # ============================================================
    # R3 C2 — RISK ADJUSTED RETURN
    # ============================================================
    with r3_c2:
        avg_ret = (
            ctx.stock_daily["close"].pct_change().mean()
            * 252
        )

        calmar = (
            round(avg_ret * 100 / dd_val, 2)
            if dd_val > 0
            else 0
        )
        ...
        rf_pct = (
            safe(
                ctx.stock_info.get("risk_free_rate_annual"),
                0.02,
            )
            * 100
        )
...
""",
    ...
    # ============================================================
    # R4 C2 — EXPLAINABLE RISK SUMMARY
    # ============================================================
    with r4_c2:
        st.markdown(
            f"""
